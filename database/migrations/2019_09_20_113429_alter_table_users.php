...
class AlterTableUsers extends Migration
{
    ...
    public function up()
    {
        ...
        foreach ($duplicateRecords as $record) {
            $users = User::where('email', 'ilike', $record->email)
                ->orderBy('id')
                ->get();
            $userTrue = $users[0];
            foreach ($users as $user) {
                if ($user->id == $userTrue->id) {
                    ...
                    $processosProcurador = Processo::where(
                        'procurador_id',
                        '=',
                        $user->id
                    )->get();
                    $processosEstagiario = Processo::where(
                        'estagiario_id',
                        '=',
                        $user->id
                    )->get();
                    $processosAssessor = Processo::where(
                        'assessor_id',
                        '=',
                        $user->id
                    )->get();
                    ...
                    $ignoredBy = Busca::where(
                        'ignored_by_id',
                        '=',
                        $user->id
                    )->get();
                    $disabledBy = User::where(
                        'disabled_by_id',
                        '=',
                        $user->id
                    )->get();
                    $revisionId = Revision::where(
                        'user_id',
                        '=',
                        $user->id
                    )->get();

                    $processosProcurador->each(function ($processo) use (
                        $userTrue
                    ) {
                    ...
                    $processosEstagiario->each(function ($processo) use (
                        $userTrue
                    ) {
                    ...
                    $processosAssessor->each(function ($processo) use (
                        $userTrue
                    ) {
